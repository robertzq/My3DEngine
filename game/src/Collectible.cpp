...
#include "Engine/ResourceManager.h" // <--- 改成相对路径
#include "Engine/Game.h"            // <--- 强制引用 Core 里的 Game.h
...
void Collectible::Render() {
    ...
    if (texture) {
        ...
        ResourceManager::DrawWhole(texture, screenRect, Game::renderer, SDL_FLIP_NONE);
    }
}
