...
VillageScene::VillageScene(std::string mapFile, bool spawnAtEntrance, int spawnX, int spawnY)
    : mapPath(mapFile), shouldSpawnAtEntrance(spawnAtEntrance),
      explicitSpawnX(spawnX), explicitSpawnY(spawnY) {}

void VillageScene::OnEnter() {
    LOG_INFO("进入地图: " << mapPath);
    ...
    int startX = 100;
    int startY = 100;
    // 优先级 0: 显式出生坐标（如战斗结束返回触发点）
    if (explicitSpawnX >= 0 && explicitSpawnY >= 0) {
        startX = explicitSpawnX;
        startY = explicitSpawnY;
    }
// === 【核心修复】优先级 1: 如果是回村庄，且有记忆坐标 ===
    else if (mapPath.find("village") != std::string::npos && g_gameState.lastVillageX != -1) {

        startX = g_gameState.lastVillageX;
        startY = g_gameState.lastVillageY;
    ...
    // 设置世界边界为地图尺寸，防止玩家走出地图
    player->SetWorldWidth(map->GetWidth() * EngineConfig::TILE_SIZE);
    player->SetWorldHeight(map->GetHeight() * EngineConfig::TILE_SIZE);
    ...
    if (mapPath.find("village") != std::string::npos && g_gameState.bossDefeatedCount >= 3) {

        // 在花圈中心 (大概是第10行, 第14列 -> x=14*32, y=10*32) 生成礼物
        // 简单起见，我暂时先用文字提示你位置
        LOG_INFO("【彩蛋】礼物盒已出现在广场中心！ 352，320");
...
void VillageScene::Update() {
    if (currentState == SceneState::PLAYING) {
        ...
        // 2. 摄像机跟随（并按地图尺寸 clamp，避免露出地图边界外的黑边）
        Game::camera.x = player->GetBounds().x - (EngineConfig::SCREEN_WIDTH / 2);
        Game::camera.y = player->GetBounds().y - (EngineConfig::SCREEN_HEIGHT / 2);
        int maxCamX = map->GetWidth() * EngineConfig::TILE_SIZE - EngineConfig::SCREEN_WIDTH;
        int maxCamY = map->GetHeight() * EngineConfig::TILE_SIZE - EngineConfig::SCREEN_HEIGHT;
        ...
        if(Game::camera.y > maxCamY) Game::camera.y = maxCamY;
        ...
        for (const auto& door : doorTriggers) {
            if (Physics::CheckCollision(player->GetBounds(), door)) {
                ...
                g_gameState.lastVillageX = player->GetBounds().x;
                g_gameState.lastVillageY = player->GetBounds().y;
                // 简单的根据坐标判断是哪个房子
        ...
        for (const auto& bossZone : bossTriggers) {
            if (Physics::CheckCollision(player->GetBounds(), bossZone)) {
                ...
                // 1. 准备参数
                        ...
                        params["return_y"] = player->GetBounds().y + EngineConfig::TILE_SIZE; // 回来时站在舞台下方，防止死循环
