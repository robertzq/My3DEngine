...
BattleScene::BattleScene(const json& params) {
    returnMap = params.value("return_map", "village.map");
    returnX = params.value("return_x", -1);
    returnY = params.value("return_y", -1);
...
void BattleScene::HandleEvents(SDL_Event& event) {
    if (currentState == PLAYER_TURN) {
        ...
        if (event.type == SDL_KEYDOWN) {
            Game::instance()->ChangeScene(new VillageScene(returnMap, false, returnX, returnY));
        }
    }
}
...
void BattleScene::Render() {
    if (bgTexture) {
        ResourceManager::DrawWhole(bgTexture, {0, 0, 800, 600}, Game::renderer, SDL_FLIP_NONE);
    } else {
    ...
    if (enemyTexture) {
        ...
        ResourceManager::DrawWhole(enemyTexture, enemyRect, Game::renderer, SDL_FLIP_NONE);
    }
    ...
    if (playerTexture) {
        ...
        ResourceManager::DrawWhole(playerTexture, playerRect, Game::renderer, SDL_FLIP_NONE);
    }
    ...
    if (uiBoxTexture) {
        ResourceManager::DrawWhole(uiBoxTexture, uiRect, Game::renderer, SDL_FLIP_NONE);
    } else {
    ...
    if (currentState == PLAYER_TURN) {
        ...
        if (cursorTexture) {
            ...
            ResourceManager::DrawWhole(cursorTexture, cursorRect, Game::renderer, SDL_FLIP_NONE);
        }
    } else {
        // 战斗信息
