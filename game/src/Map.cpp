...
Map::Map() {
    ...
    srcRect.w = destRect.w = EngineConfig::TILE_SIZE;
    srcRect.h = destRect.h = EngineConfig::TILE_SIZE;
}
...
void Map::LoadMap(std::string path) {
    ...
    LOG_INFO("地图加载成功: " << path << " (" << mapWidth << "x" << mapHeight << ")");
}
...
void Map::DrawMap() {
    for(int row = 0; row < mapData.size(); row++) {
        for(int col = 0; col < mapData[row].size(); col++) {
            ...
            destRect.x = col * EngineConfig::TILE_SIZE - Game::camera.x;
            destRect.y = row * EngineConfig::TILE_SIZE - Game::camera.y;

            // 视口剔除
            if (destRect.x < -EngineConfig::TILE_SIZE || destRect.x > EngineConfig::SCREEN_WIDTH || destRect.y < -EngineConfig::TILE_SIZE || destRect.y > EngineConfig::SCREEN_HEIGHT) continue;
            ...
            if(texToDraw)
                ResourceManager::DrawWhole(texToDraw, destRect, Game::renderer, SDL_FLIP_NONE);
        }
    }
}
...
std::vector<SDL_Rect> Map::getColliders() {
    ...
    for(int row = 0; row < mapData.size(); row++) {
        for(int col = 0; col < mapData[row].size(); col++) {
            ...
            if (isBlocking) {
                SDL_Rect collider;
                collider.x = col * EngineConfig::TILE_SIZE;
                collider.y = row * EngineConfig::TILE_SIZE;
                collider.w = EngineConfig::TILE_SIZE;
                collider.h = EngineConfig::TILE_SIZE;
    ...
    for(int row = 0; row < mapData.size(); row++) {
        for(int col = 0; col < mapData[row].size(); col++) {
            if (mapData[row][col] == tileID) {
                SDL_Rect r = {col * EngineConfig::TILE_SIZE, row * EngineConfig::TILE_SIZE, EngineConfig::TILE_SIZE, EngineConfig::TILE_SIZE};
                tiles.push_back(r);
            }
        }
    }
...
std::vector<Collectible*> Map::generateHearts() {
    ...
    for(int row = 0; row < mapData.size(); row++) {
        for(int col = 0; col <  mapData[row].size(); col++) {
            ...
            if (type == 3) {
                // 1. 计算坐标
                int x = col * EngineConfig::TILE_SIZE;
                int y = row * EngineConfig::TILE_SIZE;
