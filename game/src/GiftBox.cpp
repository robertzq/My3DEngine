...
// --- 【新增】错误检查 ---
    if (boxTexture == nullptr) {
        LOG_ERROR("Gift texture not loaded!");
    }
    if (bannerTexture == nullptr) {
        LOG_ERROR("Banner texture not loaded!");
    }
...
GiftBox::~GiftBox() {
    // 纹理由 ResourceManager 统一缓存管理，这里不能销毁，否则会造成双重释放
}
...
void GiftBox::Render() {
    ...
    if(boxTexture) {
        ...
        ResourceManager::DrawWhole(boxTexture, screenBox, Game::renderer, SDL_FLIP_NONE);
    }
    //}
    ...
    if (isOpened && bannerTexture && !isBannerDismissed) {
        ...
        // 画出来 (缩放到 400x100)
        ResourceManager::DrawWhole(bannerTexture, screenBanner, Game::renderer, SDL_FLIP_NONE);
