...
GameObject::GameObject(const char* resourceId, SDL_Renderer* ren, int x, int y, int numFrames){
    ...
    if (objTexture) {
        ...
        LOG_ERROR("GameObject 纹理加载失败 -> " << resourceId);
    }
...
void GameObject::Update() {
    ...
    // 2. 右边界限制（世界宽度由场景通过 SetWorldWidth 设置，0 表示不限制）
    if (worldWidth > 0 && xpos > worldWidth - destRect.w) {
        xpos = worldWidth - destRect.w;
    }
...
void GameObject::Render() {
    if(objTexture) {
        ...
        ResourceManager::Draw(objTexture, srcRect, screenRect, renderer, spriteFlip);
    } else {
...
GameObject::~GameObject() {
    // 纹理由 ResourceManager 统一缓存管理，这里无需释放
}