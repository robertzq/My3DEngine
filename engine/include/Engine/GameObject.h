...
public:
   ...
   virtual ~GameObject();
    ...
    // 设置世界边界（0 表示不限制），用于消除地图尺寸硬编码
    void SetWorldWidth(float w) { worldWidth = w; }
    void SetWorldHeight(float h) { worldHeight = h; }
protected:
    float xpos, ypos;      // 改成 float 以便计算精细的物理
    float velX, velY;      // 速度 (Velocity)
    float gravity;         // 重力
    float worldWidth = 0.0f;   // 世界宽度（0 = 不限制）
    float worldHeight = 0.0f;  // 世界高度（0 = 不限制）
